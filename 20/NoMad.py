...
import ctypes, string, shutil, random
...
class Ui_NoMad(object):
    global a, b, la, la_x, count_prof, ks, curtain_size, Kss#Using 'a' for Vector popup, 'b' for save toggle
    a, b, la, la_x, count_prof, ks, curtain_size, Kss = 0, 0, 4, 630, 0, 140, 320, {}
    ...
    def action(self, jsn, num):
        ...
        if "/" in jsn['image_path']:
             ...
             self.pushButton_c.setStyleSheet("QPushButton {font: 15pt \"Work Sans\" bold;\n" 
                                             f"background-color: {jsn['settings']['txt_bg']};\n"
                                             "color: white;\n border-radius: 10px;}")
    ...
    def keystrokes(self):
        ...
        self.curtain.setGeometry(QtCore.QRect(0, curtain_size, 370, 4564))
    ...
    def create_new(self):
        ...
        for d in range(1,47):
                ...
                # Kss[locals()[f'options_{d}']] = options_2
                ...
                binary_o.setObjectName(f"combobox{d}")
                setattr(self, f'binary_{d}o', binary_o)
                self.add_menu(locals()[f'K{d}'], menu_o)
                # menu_o.setObjectName(f"menu_{d}o")
                setattr(self, f'menu_{str(d)}o', menu_o)

                toolButton_o = QtWidgets.QToolButton(options_2)
                setattr(self, f'options_{str(d+1)}', options_2)
                ...
                # Kss[locals()[f'toolButton_{d}o']] = toolButton_o
                # toolButton_o.setObjectName("toolButton_o")
                setattr(self, f'toolButton_{str(d)}o', toolButton_o)
                ...
                lineEdit_2o.setObjectName(f"delay{d}")
                setattr(self, f'lineEdit_{d}o', lineEdit_2o)
...
#         self.addKeystrokeButton_o.setVisible(False)
    ...
    def save_profile(self):
        ...
        settings["txt_bg"] = f"#{random.randint(111111,999999)}"
    ...
    def layer(self,num):
        ...
        for specific_element in self.frame_2.children():
             ...
             val = str(val)
             if val in list(loaded_dict[f'layer{num}']):
                  specific_element.setText(loaded_dict[f'layer{num}'][val])
...
#         # if os.path.isfile(self.resource_path('layer1.json')):
#         with open(self.resource_path('layer1.json'), 'r') as js_file:
#                 loaded_dict = json.load(js_file)
#         # if len(loaded_dict) > 1:
#         print(loaded_dict['layer2'])
#         for key, value in loaded_dict['layer2'].items():
#              print(type(key), type(value))
#              self.previous_text_L2[key] = value
        # print(self.previous_text_L2)
        
        

        # else:
        #         print('agya/ reached else')
        #         # Save elements to retrive easily
        #         for element in self.frame_2.children():
        #              if isinstance(element, QtWidgets.QWidget):  # Check if the element is a widget
        #                   if hasattr(element, "text"):  # Check if the widget has a text attribute
        #                        self.previous_text_L2[element.x()+element.y()] = element.text()  # Store the previous text


        # data1 = {}
        # data1['layer2'] = self.previous_text_L2
        # with open(self.resource_path('layer1.json'), 'w') as js_file:
        #      json.dump(data1, js_file)
...
#         for key, value in loaded_dict['layer3'].items():
#              self.previous_text_L3[key] = value
...
#                                self.previous_text_L3[element.x()+element.y()] = element.text()  # Store the previous text
    ...
    def basic_func(self, clicked): 
        ...
        for element in self.frame_2.children():
            if isinstance(element, QtWidgets.QPushButton) and hasattr(element, "text"):
                ...
                #     self.previous_text_L1[val] = clicked  # Update the button text
                #     exec(f"{var_name} = clicked")
                    print('basic_func')
    ...
    def binary_search_replace(self, key, new_value, num):
        ...
        replacements = {'': 0, 'A': 4, 'B': 5, 'C': 6, 'D': 7, 'E': 8, 'F': 9, 'G': 10, 'H': 11, 'I': 12, 'J': 13, 'K': 14, 'L': 15, 'M': 16, 'N': 17, 'O': 18, 'P': 19, 'Q': 20, 'R': 21, 'S': 22, 'T': 23, 'U': 24, 'V': 25, 'W': 26, 'X': 27, 'Y': 28, 'Z': 29,
                        '!\n \n1': 30, '@\n \n2': 31, '#\n \n3': 32, '$\n \n4': 33, '$\n \n5': 34, '%\n \n6': 35, '^\n \n7': 36, '*\n \n8': 37, '(\n \n9': 47, '( 9': 47, ')\n \n0': 48, ') 0': 48, 'Enter': 40, 'Esc': 41, 'Backspace': 42, 'Tab': 43, 'Space': 44, '-\n \n_': 45,
...
#         self.lineedit_c = QtWidgets.QLineEdit(self.CreateAction)
#         self.lineedit_c.setGeometry(QtCore.QRect(10, 140, 361, 31))
#         self.lineedit_c.setMinimumSize(QtCore.QSize(0, 0))
#         self.lineedit_c.setMaximumSize(QtCore.QSize(16777215, 13516535))
#         font = QtGui.QFont()
#         font.setFamily("Work Sans Light")
...
"}")
    ...
    def retranslateUi(self, NoMad):
        ...
        # self.previous_text_L1[key] = value
        # self.previous_text_L1 = loaded_dict
        ...
        #              if isinstance(element, QtWidgets.QWidget):  # Check if the element is a widget
        #                   if hasattr(element, "text"):  # Check if the widget has a text attribute
        #                        self.previous_text_L1[element.x()+element.y()] = element.text()  # Store the previous text
        
        # data1 = {}
        # data1['layer1'] = self.previous_text_L1
        # with open(self.resource_path('layer1.json'), 'w') as js_file:
        #      json.dump(data1, js_file)
