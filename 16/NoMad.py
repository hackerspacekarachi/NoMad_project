...
class Ui_NoMad(object):
    global a, b, la, la_x, count_prof #Using 'a' for Vector popup, 'b' for save toggle
    a, b, la, la_x, count_prof = 0, 0, 4, 630, 0
    ...
    def setupUi(self, NoMad):
        ...
        self.pushButton_3c = QtWidgets.QPushButton(self.CreateAction, clicked = lambda: self.save_actions(self.commandLinkButton_6.text()))
        ...
        for element in self.frame_2.children():
            if isinstance(element, QtWidgets.QWidget):  # Check if the element is a widget
                if hasattr(element, "text"):  # Check if the widget has a text attribute
                    self.previous_text[element] = element.text()  # Store the previous text
