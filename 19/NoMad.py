...
#                                self.previous_text_L3[element.x()+element.y()] = element.text()  # Store the previous text
    ...
    def binary_search_replace(self, key, new_value, num):
        ...
        # if not os.path.exists('layer1.json'):
        #      with open(self.resource_path('layer1.json'), 'w') as file:
        #           file.write('{}')

        with open(self.resource_path('layer.json'), 'r') as file:
             data = json.load(file)

        for i in list(data[f'layer{num}'].keys()):
             if i == str(key):
                  data[f'layer{num}'][key] = new_value

        # print(data)
        # data1 = {}
        # data1[f'layer{num}'] = data
