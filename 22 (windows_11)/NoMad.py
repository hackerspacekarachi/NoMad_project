...
class Ui_NoMad(object):
    ...
    def openCreateAction(self):
        ...
        # SCROLLBAR ON
        self.main_frame.setGeometry(QtCore.QRect(0, 0, 1337, 400))
        self.main_frame.setMinimumSize(QtCore.QSize(1337, 400))
        self.main_frame.setMaximumSize(QtCore.QSize(1337, 400))
        self.keyboard_area.setGeometry(QtCore.QRect(0, 0, 1337, 400))
        self.scrollArea_keyboard.horizontalScrollBar().setVisible(True)
    ...
    def save_toggle(self):
        global b
        b +=1
        ratio = (148 * (NoMad.frameGeometry().width()//2)) // 586
    ...
    def action(self, jsn, num):
        ...
        if "/" in jsn['image_path']:
             ...
             self.pushButton_c.setStyleSheet("font: 13pt \"Work Sans\" bold;\n background-color: {} ;\n color: white;\n border-radius: 10px;".format(jsn['settings']['txt_bg']))
    ...
    def create_new(self):
        ...
        self.label_o.setStyleSheet("font: 6pt \"Work Sans Light\";")
    ...
    def multi_action(self):
        ...
        self.lineEdit_o.setObjectName("action")
        self.checkBox_o = QtWidgets.QCheckBox(self.create_area)
        self.checkBox_o.setGeometry(QtCore.QRect(102, 70, 111, 21))
        font = QtGui.QFont()
        font.setFamily("Work Sans Light")
        font.setPointSize(6)
    ...
    def basic_keys(self):
        ...
        self.h1.setStyleSheet("font: 7pt \"Work Sans\";")
        ...
        self.h2.setStyleSheet("font: 7pt \"Work Sans\";")
        ...
        self.h3.setStyleSheet("font: 7pt \"Work Sans\";")
        ...
        self.h4.setStyleSheet("font: 7pt \"Work Sans\";")
        ...
        self.h5.setStyleSheet("font: 7pt \"Work Sans\";")
        ...
        self.h6.setStyleSheet("font: 7pt \"Work Sans\";")
        ...
        self.h7.setStyleSheet("font: 7pt \"Work Sans\";")
    ...
    def profiles(self):
            ...
            for j in os.listdir('Actions'):
                for i in os.listdir('Actions/'+j):
                    if i.endswith(".json"):
                        ...
                        self.pushButton_.clicked.connect(self.Vect_art)
        ...
        self.pushButton_0 = QtWidgets.QPushButton(self.frame_5, clicked= lambda: self.saveWindow())
        self.pushButton_0.setGeometry(QtCore.QRect(10, f5_y, 130, 25))
        self.pushButton_0.setText("+ Add new")
        self.pushButton_0.clicked.connect(self.Vect_art)
    ...
    def layer(self,num):
        ...
        one = self.commandLinkButton_6.text()
        two = self.pushButton_85.palette().button().color().name()
        data = {'profile_name': one, 'color': two, 'layer_n': num, 'layer_name': 'Quick brush'}

        ref = db.reference("INPUT/")
        loaded_dict["profile"] = data
        loaded_dict = self.hid(loaded_dict)
        ref.update(loaded_dict)
    ...
    def hid(self, data):
        ...
        replacements = {'': 0, 'A': 4, 'B': 5, 'C': 6, 'D': 7, 'E': 8, 'F': 9, 'G': 10, 'H': 11, 'I': 12, 'J': 13, 'K': 14, 'L': 15, 'M': 16, 'N': 17, 'O': 18, 'P': 19, 'Q': 20, 'R': 21, 'S': 22, 'T': 23, 'U': 24, 'V': 25, 'W': 26, 'X': 27, 'Y': 28, 'Z': 29,
                        '!\n \n1': 30, '@\n \n2': 31, '#\n \n3': 32, '$\n \n4': 33, '%\n \n5': 34, '^\n \n6': 35, '&&\n \n7': 36, '*\n \n8': 37, '(\n \n9': 38, '( 9': 38, ')\n \n0': 39, ') 0': 39, 'Enter': 40, 'Esc': 41, 'Backspace': 42, 'Tab': 43, 'Space': 44, ' ': 44, '-\n \n_': 45,
                        '+\n \n=': 46, '|\n \n\\': 49, '\\': 49, '#': 50, ':\n \n;': 51, '" \n \n\'': 52, '~\n \n`': 53, '<\n \n,': 54, '>\n \n.': 55, '?\n \n/': 56, 'Caps Lock': 57, 'F1': 58, 'F2': 59, 'F3': 60, 'F4': 61, 'F5': 62, 'F6': 63, 'F7': 64, 'F8': 65,
                        'F9': 66, 'F10': 67, 'F11': 68, 'F12': 69, 'Print\nscreen': 70, 'Scroll\nlock': 71, 'Pause': 72, 'Insert': 73, 'Home': 74, 'Page\nUp': 75, 'Del': 76, 'End': 77, 'Page\nDown': 78, '\u2192': 79, '\u2190': 80, '\u2193': 81, '\u2191': 82, 'Num\nlock': 83,
                        '/': 84, '*': 85, '-': 86, '+': 87, 'Numpad Enter': 88, 'Numpad 1': 89, 'Numpad 2': 90, 'Numpad 3': 91, 'Numpad 4': 92, 'Numpad 5': 93, 'Numpad 6': 94, 'Numpad 7': 95, 'Numpad 8': 96, 'Numpad 9': 97, 'Numpad 0': 98, '.': 99, '~': 100,
                        'Fn': 101, 'Power': 102, '=': 103, 'F13': 104, 'F14': 105, 'F15': 106, 'F16': 107, 'F17': 108, 'F18': 109, 'F19': 110, 'F20': 111, 'F21': 112, 'F22': 113, 'F23': 114, 'F24': 115, 'Exec': 116, 'Help': 117, 'Menu': 118, 'Select': 119,
                        'Stop': 120, 'Again': 121, 'Undo': 122, 'Cut': 123, 'Copy': 124, 'Paste': 125, 'Find': 126, 'Mute': 127, 'Vol+': 128, 'Vol-': 129, 'Locking\nCaps\nLock': 130, 'Locking\nNum\nLock': 131, 'Locking\nScroll\nLock': 132, ',': 133, '{\n \n[': 47,
                        '}\n \n]': 48, '\\': 49, "'": 52, 'Caps\nlock': 57, 'Print Screen': 70, 'Scroll Lock': 71, 'Home Page': 74, 'Pag up': 75, 'Pag down': 78, 'Num Lock': 83, 'Execute': 116, 'Volume Mute': 127, 'Volume Up': 128, 'Volume Down': 129,
                        'Locking Caps Lock': 130, 'Locking Num Lock': 131, 'Locking Scroll Lock': 132, 'Alt\nErase': 153, 'Clear': 156, 'Return': 158, 'Left\nCtrl': 224, 'Ctrl': 224, 'Left\nShift': 225, 'Shift': 225, 'Left\nAlt': 226, 'Cmd': 226, 'Left\nWin': 227, 'Opt': 227, 'Right\nCtrl': 228, 'Right\nShift': 229,
                        'Right\nAlt': 230, 'Right\nWin': 231}

        replace_values_recursive(data, replacements)

        return data
    

    def firebase(self, data):
        global push_data        # HID decimals to global variable

        push_data = self.hid(data)

        one = self.commandLinkButton_6.text()
        two = self.pushButton_85.palette().button().color().name()
        three = [t.text() for t in self.tab.buttons() if t.isChecked()][0]
        push_data['profile'] = {'profile_name': one, 'color': two, 'layer_n': three, 'layer_name': 'Quick brush'}

        ref = db.reference("INPUT/")
        ref.update(push_data)
    ...
    def setupUi(self, NoMad):
        cred = credentials.Certificate(self.resource_path("serviceAccount1.json"))
        firebase_admin.initialize_app(cred, {"databaseURL": "https://input-39f4e-default-rtdb.firebaseio.com/"})
        ...
        NoMad.setGeometry(QtCore.QRect(int(ax*0.106), int(wae*0.065), 1079, 623))       #QtCore.QRect(0, 0, 1079, 623)
...
"}")
        ...
        self.label_c.setStyleSheet("font: 6pt \"Work Sans Medium\";")
...
"}")
        ...
        self.label_2o.setStyleSheet("font: 6pt \"Work Sans Light\";")
...
"border: none;")
        ...
        self.main_frame.setGeometry(QtCore.QRect(0, 0, 1337, 400)) # 1079, 623
        self.main_frame.setStyleSheet("QFrame { background-color: no color;\n"
                                        "border: none;\n"
                                        "} \n"
        ...
        self.main_frame.setMinimumSize(QtCore.QSize(1079, 400)) # 1337, 400
        self.main_frame.setMaximumSize(QtCore.QSize(1079, 400))
        self.main_frame.setObjectName("main_frame")
        self.boxLayout.addWidget(self.main_frame,0,QtCore.Qt.AlignCenter) #,0,QtCore.Qt.AlignCenter
        self.boxLayout.addStretch()

        self.keyboard_area = QtWidgets.QWidget(self.main_frame)
        self.keyboard_area.setGeometry(QtCore.QRect(0, 0, 1337, 400))
        ...
        self.scrollArea_keyboard.setStyleSheet("QScrollArea {\n"
                                        "background-color: no color;\n"
                                        "border: none;\n"
                                        "}\n")
        self.scrollArea_keyboard.horizontalScrollBar().setVisible(False)
        # vbar = self.scrollArea_keyboard.verticalScrollBar()
        # self.scrollArea_keyboard.ensureVisible(50, 100)
        # vbar.setValue(vbar.maximum())
        # self.scrollArea_keyboard.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarAlwaysOff)
        # self.scrollArea_keyboard.horizontalScrollBar().valueChanged(50)
        self.scrollArea_keyboard.setWidgetResizable(True)
        self.scrollArea_keyboard.setObjectName("scrollArea_keyboard")
        self.scrollWidget_keyboard = QtWidgets.QWidget(self.scrollArea_keyboard) #self.scrollArea_c
        ...
        # self.abc = QtWidgets.QScrollBar(self.scrollArea_keyboard)
        # self.abc.setOrientation(QtCore.Qt.Horizontal)
        # self.abc.setSliderPosition(50)
        # self.abc.setInvertedControls(False)
        # self.abc.setSingleStep(4)
        ...
        # self.abc.setWidget(self.scrollWidget_keyboard)
        # self.horizLayout_keyboard.addWidget(self.abc)
        

        # self.profiles_area_2 = QtWidgets.QFrame() # We will use frame_2
...
"border-radius: 20px;")
        ...
        self.keyboardLayout.addWidget(self.frame_2,0,QtCore.Qt.AlignLeft) #,0,QtCore.Qt.AlignCenter
...
"}")
        ...
        self.pushButton_98.clicked.connect(self.save_toggle)
...
"border-radius: none;")
        ...
        self.pushButton_85.setStyleSheet("background-color: rgba(255, 0, 0, 0);\n"         #c0c0c0
"font: 6pt \"Arial\";\n"
"color: rgb(0, 0, 0);\n"
"border-style: none;\n"
"border-radius: 12px;")
        ...
        if current_size != self.last_size:
            if  self.CreateAction.frameGeometry().width() != 0:
                ...
                if self.options.frameGeometry().width() != 0:
                        ...
                        self.label_2o.setGeometry(QtCore.QRect(140, NoMad.frameGeometry().height()-210-40, 111, 16)) # -60
                        self.category_o.setGeometry(QtCore.QRect(30, NoMad.frameGeometry().height()-190-40, 301, 30)) # -40
                        self.pushButton_2o.setGeometry(QtCore.QRect(90, NoMad.frameGeometry().height()-150-40, 191, 31))
        
            self.last_size = current_size

    def auto_click_button(self):
        #  self.scrollWidget_keyboard.grab()
         QtTest.QTest.mouseClick(self.last_frame, QtCore.Qt.LeftButton)
